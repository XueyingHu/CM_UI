...
const EXTRACTED: Array<{
  filename: string;
  events: { type: string; description: string; rating: "Critical" | "Major" | "Limited" }[];
}> = [
  {
    filename: "Ops Risk Summary.docx",
    events: [
      { type: "Risk Event", description: "System outage impacting payments", rating: "Critical" },
      { type: "Risk Event", description: "Processing delays due to vendor capacity", rating: "Major" },
      { type: "Issue", description: "Access control gaps in core platform", rating: "Major" },
      { type: "Issue", description: "Incomplete evidence retention for reconciliations", rating: "Limited" },
      { type: "Change", description: "Payments platform upgrade", rating: "Major" },
      { type: "Change", description: "Risk controls monitoring automation rollout", rating: "Limited" },
      { type: "Unmatched", description: "Internal Risk Event, third party processing delay", rating: "Major" },
      { type: "Unmatched", description: "Known Issue, legacy reconciliation workaround", rating: "Limited" },
    ],
  },
  {
    filename: "Incident Report.pdf",
    events: [
      { type: "Risk Event", description: "Data integrity failure in reporting pipeline", rating: "Critical" },
      { type: "Issue", description: "Manual override procedure not documented", rating: "Major" },
      { type: "Change", description: "Core banking system refresh", rating: "Major" },
      { type: "Unmatched", description: "Regulatory Exam, Q4 review findings", rating: "Major" },
    ],
  },
  {
    filename: "Ops Workflow.vsdx",
    events: [
      { type: "Change", description: "Workflow automation for ops reconciliation", rating: "Limited" },
      { type: "Unmatched", description: "Significant Organization Change, team restructure Q1", rating: "Major" },
      { type: "Unmatched", description: "Business Ownership Change, new ops lead", rating: "Limited" },
    ],
  },
];
...
const TYPE_STYLES: Record<EventType, { bg: string; border: string; color: string; label: string }> = {
  "Risk Event": { bg: "rgba(180,35,24,0.06)", border: "rgba(180,35,24,0.22)", color: "#b42318", label: "Risk Event" },
  "Issue":      { bg: "rgba(181,71,8,0.07)",  border: "rgba(181,71,8,0.25)",  color: "#b54708", label: "Issue" },
  "Change":     { bg: "rgba(31,94,168,0.07)", border: "rgba(31,94,168,0.22)", color: "#1f5ea8", label: "Change" },
  "Unmatched":  { bg: "rgba(90,90,90,0.06)",  border: "rgba(90,90,90,0.20)",  color: "#4a5568", label: "Needs Validation" },
};
...
function TypeBadge({ type }: { type: EventType }) {
  ...
  return (
    <span style={{
      ...
      padding: "3px 8px", borderRadius: 999, fontSize: 11.5, fontWeight: 900,
...
export default function Step2Extract() {
  ...
  const confPct  = extractResult ? Math.round(extractResult.overall_confidence * 100) : null;
  const barColor = confPct != null
    ? (confPct >= 90 ? "#1f7a3f" : confPct >= 75 ? "#b54708" : "#b42318")
    : "#1f5ea8";
  const barBg = confPct != null
    ? (confPct >= 90 ? "rgba(31,122,63,0.10)" : confPct >= 75 ? "rgba(181,71,8,0.10)" : "rgba(180,35,24,0.10)")
    : "rgba(31,94,168,0.10)";
      ...
      <div style={{ padding: 18, flex: 1 }}>
        <div style={{
          ...
          <div style={{ marginBottom: 14 }}>
            <h1 style={{ margin: "0 0 5px", fontSize: 16, fontWeight: 900, color: TEXT }}>
              Step 2. Extract Key Events
            </h1>
            <p style={{ margin: 0, fontSize: 12.8, color: MUTED, lineHeight: 1.4 }}>
              The AI has identified the following events, issues, and changes from your uploaded documents.
              Items marked "Needs Validation" were referenced in the documents but could not be matched to a known source system.
            </p>
          </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontWeight: 900, fontSize: 13, color: TEXT }}>
                    ...
                    <span style={{ color: "#1f5ea8" }}>
                      {extractResult.documents_processed} of {extractResult.total_documents} document{extractResult.total_documents !== 1 ? "s" : ""} processed,&nbsp;
            ...
            {EXTRACTED.map((doc) => (
              <div key={doc.filename}>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 12.5, fontWeight: 900, color: "#1f5ea8" }}>{doc.filename}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: MUTED }}>
                    — {doc.events.length} item{doc.events.length !== 1 ? "s" : ""} identified
                  </span>
                </div>
                    ...
                    <thead>
                      <tr>
                        {["Type", "Description", "Rating"].map(h => (
                          <th key={h} style={{
                            padding: "9px 12px", background: "#f7f9fd", fontWeight: 900,
                            fontSize: 12.5, color: "#334155", textAlign: "left",
                    ...
                    <tbody>
                      {doc.events.map((ev, i) => {
                        ...
                        const td: React.CSSProperties = {
                          ...
                          verticalAlign: "middle", color: TEXT,
                          borderBottom: isLast ? "none" : `1px solid #eef2f7`,
                        };
                        return (
                          <tr key={i} style={{ background: ev.type === "Unmatched" ? "rgba(248,248,248,0.7)" : "#fff" }}>
                            <td style={{ ...td, whiteSpace: "nowrap" }}>
                              <TypeBadge type={ev.type as EventType} />
                            </td>
                            <td style={{ ...td, color: ev.type === "Unmatched" ? "#5b6b7a" : TEXT, fontStyle: ev.type === "Unmatched" ? "italic" : "normal" }}>
                              {ev.description}
                            </td>
                            <td style={{ ...td, whiteSpace: "nowrap", fontWeight: 900, color: RATING_STYLES[ev.rating].color }}>
                              {ev.rating}
                            </td>
                          </tr>
                        );
